...
typedef struct {
    ...
    ota_status_callback_t callback;      // 状态回调函数
...
esp_err_t ota_begin(uint32_t total_size)
{
    if (ota_ctx.state != OTA_STATE_IDLE) {
        ...
        if (ota_ctx.callback) {
            ota_ctx.callback(OTA_STATE_ERROR, 0);
        }
    ...
    if (ota_ctx.callback) {
        ota_ctx.callback(OTA_STATE_RECEIVING, 0);
    }
...
esp_err_t ota_write(const uint8_t *data, size_t len)
{
    ...
    // 回调进度更新
    if (ota_ctx.callback) {
        ota_ctx.callback(OTA_STATE_RECEIVING, progress);
    }
    ...
    if (ota_ctx.received_size >= ota_ctx.total_size) {
        ...
        if (ota_ctx.callback) {
            ota_ctx.callback(OTA_STATE_VALIDATING, 100);
        }
    }
...
esp_err_t ota_end(void)
{
    ...
    if (ota_ctx.callback) {
        ota_ctx.callback(OTA_STATE_UPDATING, 100);
    }
    ...
    if (ota_ctx.callback) {
        ota_ctx.callback(OTA_STATE_SUCCESS, 100);
    }
